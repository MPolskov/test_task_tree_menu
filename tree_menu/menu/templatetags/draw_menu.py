...
def draw_menu(context, menu):
    ...
    menu_items = (
        MenuItem.objects.filter(menu__slug=menu).select_related('menu')
    )
    ...
    for item in base_items:
        if item['id'] in selected_items_list:
            item['children'] = get_children(menu_items,
                                            selected_items_list,
                                            item['id'])
    result = {
        'items': base_items,
...
def get_item_list(selected_item):
    """Вспомогательная функция для определения узловых пунктов меню."""
    node_items_list = []
    while selected_item:
...
def get_children(menu_items, selected_items_list, item_id):
    """Вспомогательная функция для определения вложенности."""
    children_list = (
        [item for item in menu_items.filter(parent_id=item_id).values()]
    )
